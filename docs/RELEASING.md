...
A release is a signed, notarized macOS build attached to a GitHub
Release. Pushing a tag does the work; a human writes the notes and
clicks publish.

The whole thing is [`.github/workflows/release.yml`](../.github/workflows/release.yml).
...
You get an unsigned build in `release/`. That is fine for testing your
own changes, and it is what happens on a machine with no certificate.
macOS will warn on it, which is correct: it is not signed.

To sign locally, export the same variables the workflow uses and run the
same command. It is rarely worth it. The runner is a clean machine, which
is the point.
...
- Windows and Linux builds. The packager is macOS-only today.
- Auto-update. `electron-updater` is not wired in, though the release
  layout (GitHub provider, `.zip` alongside the `.dmg`) is what it would
  expect, so adding it later does not require redoing this.
