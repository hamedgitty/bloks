...
describe("the skill catalog", () => {
  ...
  test("the real catalog we publish is readable and complete", async () => {
    // the file that ships with the site, checked here rather than
    // discovered broken by somebody browsing
    const raw = readFileSync(join(process.cwd(), "site/public/skills/index.json"), "utf8");
    const parsed = JSON.parse(raw);
    assert.ok(Array.isArray(parsed.skills) && parsed.skills.length >= 5);
    for (const skill of parsed.skills) {
      assert.match(skill.id, /^[a-z0-9][a-z0-9-]*$/);
      assert.ok(skill.name && skill.description && skill.version);
      assert.ok(skill.body.length > 100, `${skill.id} should have real instructions`);
      assert.ok(!/—|–/.test(`${skill.name}${skill.description}${skill.body}`), `${skill.id} has a dash`);
    }
    const ids = parsed.skills.map((s: any) => s.id);
