...
describe("the rule that keeps it that way", () => {
  ...
  test("the marketing site's copy of the palette still matches", () => {
    // site/public/index.html mocks the app with its own tokens, and its
    // own comment says every colour is lifted from src/styles.css. That
    // stops being true the moment a token moves, and the page is the
    // first thing anybody sees.
    const page = readFileSync(join(repo, "site", "public", "index.html"), "utf8");
    const block = page.match(/\.demo \{([\s\S]{0,400}?)\}/);
    assert.ok(block, "the site's token block moved and this test cannot find it");
    for (const [name, want] of [
      ["--w-mf", token("muted-foreground", "light")],
      ["--w-brand", token("brand", "light")],
      ["--w-ink", token("brand-ink", "light")],
      ["--w-soft", token("brand-soft", "light")],
      ["--w-warn", token("warning", "light")],
      ["--w-ok", token("success", "light")],
    ] as const) {
      const found: RegExpMatchArray | null = block![1].match(
        new RegExp(`${name}\\s*:\\s*(#[0-9a-fA-F]{6})`),
      );
      assert.ok(found, `the site has no ${name}`);
      assert.equal(
        found![1].toLowerCase(),
        want.toLowerCase(),
        `the site's ${name} is ${found![1]} and the palette says ${want}`,
      );
    }
  });
