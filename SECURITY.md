...
live in `~/.bloks` on your machine. Bloks has no backend of its own. The
only outbound traffic is what the provider CLIs (Claude Code, Codex) and
any connectors you configure make on their own.
...
**Released builds are signed and notarized.** Anything on the Releases
page is built by CI from a tag, signed with a Developer ID certificate,
and notarized by Apple, so it opens without a Gatekeeper warning and can
be revoked if a build is ever found to be compromised. A build you make
yourself with `pnpm package` is unsigned, and macOS will say so. See
